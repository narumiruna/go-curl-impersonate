...
目前 repository 幾乎是空專案，只包含 `LICENSE`。本計畫先以可落地的 Go/cgo wrapper 為核心，逐步補齊建置、API、測試與發佈流程。
...
- [ ] 從 `.refs/curl-impersonate/README.md#libcurl-impersonate` 與 header/cdef 來源確認 `curl_easy_impersonate` symbol、library names、non-standard options；verify with `docs/native-api.md` 記錄 C function signature、linking target、必需 headers 與最低版本。
- [ ] 決定 native dependency 策略，實作 `pkg-config` 或明確的 `CGO_CFLAGS` / `CGO_LDFLAGS` 偵測文件；verify with `docs/build.md` 可在 Linux amd64 從乾淨環境完成安裝與 `go test ./...`。
...
- [ ] 加入並發與 handle reuse 策略，明確定義 client 是否 goroutine-safe；verify with `go test -race ./...` 與文件中的 concurrency contract。
